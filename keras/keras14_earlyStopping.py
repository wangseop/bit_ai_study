...
early_stopping = EarlyStopping(monitor='val_loss', patience=500, mode='auto')
#model.fit(x, y, epochs=100)
# model.fit(x_train, y_train, epochs=100, batch_size=1)
model.fit(x_train, y_train, epochs=10000, batch_size=5, validation_data= (x_val, y_val), callbacks=[early_stopping])

#4. 평가 예측
loss, acc = model.evaluate(x_test, y_test, batch_size=5)    # 평가 지표
print('acc : ', acc)
